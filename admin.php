...
<style type="text/css">
  ...
  .dropdown-customer-btn:hover,
  .dropdown-seller-btn:hover{
    background-color: green;
  }
...
<nav role='navigation'>
  <ul class="main">
    ...
    <button class="dropdown-customer-btn" onclick="changecustomercarret();"><i class="fas fa-users" style=" margin-right: 8px"></i>
      <font size="3">Customer</font><i class="fa fa-caret-down" id="customercarret"></i>
    </button>
    ...
    <button class="dropdown-seller-btn" onclick="changesellercarret();"><i class="fas fa-users" style=" margin-right: 8px"></i>
      <font size="3">Seller</font><i class="fa fa-caret-down" id="sellercarret"></i>
    </button>

    <div class="sellercontainter">

      <li class="viewseller"><a href="#">View Seller</a></li>
      <li class="deleteseller"><a href="#">Delete Seller</a></li>
      ...
      <script>
        $('.viewseller').click(function() {
          showfield('view_seller');
        });

        $('.deleteseller').click(function() {
          showfield('delete_seller');
        });




        function changesellercarret() {
          var element = document.getElementById("sellercarret");
        ...
        /* Loop through all dropdown buttons to toggle between hiding and showing its dropdown content - This allows the user to have multiple dropdowns without any conflict */
        var dropdown = document.getElementsByClassName("dropdown-seller-btn");
        var i;
        for (i = 0; i < dropdown.length; i++) {
          dropdown[i].addEventListener("click", function() {
            this.classList.toggle("active");
  ...
  <section class="panel important">
    <body onload="hidefield()">
      ...
      <div id="ds" style="display: none">
        <form method="POST" action="delete_seller.php">
          <center>
            <input type="text" min="0" name="usr_id" placeholder="Enter User ID" /> </br>
      ...
      <form method="POST" action="add_product.php" enctype="multipart/form-data">
        <center>
          ...
          <input type="file" name="pic" id="pic" accept="image/jpeg, image/png, image/jpg"><br>
      ...
      <div id="cp" style="display:none">
        <center>
          <form method="POST" action="updateadminpassword.php">
            ...
            <input type="password" id="passbb" required name="confirmpassword" placeholder="Confirm New Password" onchange="checkpass();" onkeyup="checkpass();" />
            <div id="notmat" style="color:rgb(255,0,0);display:none"> Password Not Matching </div>
            <div id="mat" style="color:rgb(0,255,0);display:none"> Password Matching </div> </br>
      ...
      <center>
          <table>
            ...
            <?php
            $users = 'SELECT * FROM sellers';
            $result = mysqli_query($conn, $users);
            if ($result->num_rows == 0) {
              echo "No Sellers found. <br>.";
            } else {
              while ($row = $result->fetch_assoc()) {
                $s_id = $row['usr_id'];
                $s_fn = $row['first_name'];
                $s_ln = $row['last_name'];
                $s_username = $row['usr_name'];
                $user_id_s = '<a href="javascript:var z=confirmdeletioncustomer(); if(z == true){window.location.href = \'removecustomerfromid.php?username=' . $s_id . '&\';} "><font color=rgb(255,255,255)>' . $s_id . '</font></a>';
                $fn_s = '<a href="javascript:var s=confirmdeletioncustomer(); if(s == true){window.location.href = \'removecustomerfromid.php?username=' . $s_id . '&\';}"><font color=rgb(255,255,255)>' . $s_fn . '</a></font>';
                $ln_s = '<a href="javascript:var t=confirmdeletioncustomer(); if(t == true){window.location.href = \'removecustomerfromid.php?username=' . $s_id . '&\';}"><font color=rgb(255,255,255)>' . $s_ln . '</a></font>';
                $username_s = '<a href="javascript:var q=confirmdeletioncustomer(); if(q == true){window.location.href = \'removecustomerfromid.php?username=' . $s_id . '&\';}"><font color=rgb(255,255,255)>' . $s_username . '</a></font>';

                echo '<tr><td>' . $user_id_s . '</td><td>' . $fn_s . '</td><td>' . $ln_s . '</td><td>' . $s_username . '</td> </tr> ';
              }
            }
            ?>
          </table>
